  ...
  if (items.length === 0) {
    return (
      <div className="bg-white dark:bg-surface-lowest min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Seu carrinho está vazio</h2>
        <p className="text-slate-500 dark:text-slate-400 mt-2">Adicione produtos do catálogo para começar seu pedido.</p>
        <button 
          onClick={onBack}
  ...
  return (
    <div className="bg-surface min-h-screen pb-40">
      <header className="px-4 py-6 flex items-center gap-4 bg-white dark:bg-surface-lowest sticky top-0 z-30 shadow-sm">
        ...
        <h1 className="text-lg font-bold text-slate-900 dark:text-slate-100">Meu Carrinho</h1>
      ...
      <main className="px-4 mt-6 space-y-4">
        {items.map((item) => (
          <div key={item.product.id} className="card p-4 flex gap-4">
            ...
            <div className="flex-grow min-w-0">
              <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 truncate">{item.product.name}</h3>
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">R$ {item.priceAtQty.toFixed(2)} / un</p>
              ...
              <div className="mt-3 flex items-center justify-between">
                <div className="flex items-center bg-surface-low rounded-lg p-1">
                  ...
                  <span className="px-3 text-sm font-bold text-slate-900 dark:text-slate-100 min-w-[32px] text-center">
                    {item.quantity}
                  </span>
                  <button 
                    onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
        ...
        <section className="bg-white dark:bg-surface-lowest rounded-2xl p-6 mt-8 space-y-4 shadow-sm">
          <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 uppercase tracking-widest text-[10px]">Resumo do Pedido</h3>
          
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-slate-500 dark:text-slate-400">Subtotal</span>
              <span className="font-bold text-slate-900 dark:text-slate-100">R$ {subtotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-sm">
              ...
              <span className={frete === 0 ? 'text-green-600 font-bold' : 'font-bold text-slate-900 dark:text-slate-100'}>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white dark:bg-surface-lowest px-4 py-6 border-t border-surface-low shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
        <button 
          onClick={onCheckout}
