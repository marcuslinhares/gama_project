  ...
  if (loading) {
    return (
      <div className="p-4 space-y-4">
        <h1 className="text-xl font-black text-slate-900 mb-6">Meus Pedidos</h1>
        {[1, 2, 3].map(i => (
          <div key={i} className="h-32 bg-white rounded-2xl animate-pulse border border-surface-low" />
        ))}
      </div>
    );
  }
    ...
    <div className="pb-24 p-4">
      ...
      {orders.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
          ...
          <p className="text-slate-400 font-bold">Você ainda não fez nenhum pedido.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {orders.map(order => {
            ...
            return (
              <div key={order.id} className="bg-white rounded-2xl p-5 shadow-sm border border-surface-low relative active:scale-[0.98] transition-all">
                ...
                <div className="flex justify-between items-end">
                  <div>
                    <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-0.5">Total</span>
