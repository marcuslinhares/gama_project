...
interface Product { id: string; name: string; sku: string; category: string; stock: number; unitPrice: number; tieredPricing: TieredPrice[]; image?: string; }
    ...
    <div onClick={onClick} className="card p-4 flex flex-col h-full group cursor-pointer active:scale-[0.98] transition-all">
      ...
      <div className="mt-4 flex items-end justify-between">
        <div>
          ...
          <span className="text-lg font-bold text-primary">R$ {Number(product.tieredPricing[product.tieredPricing.length - 1].price).toFixed(2)}</span>
